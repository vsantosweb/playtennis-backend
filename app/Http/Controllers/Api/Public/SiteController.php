...
class SiteController extends Controller
{
    ...
    public function showProduct($slug)
    {
        ...
        switch ($business) {
            ...
            case 'assinaturas':
                $subscription = Subscription::where('slug', $slug)->with('gyms')->firstOrFail();
                return $this->outputJSON(new  SubscriptionResource($subscription));
            case 'locacoes':
                $lease = Lease::where('slug', $slug)->with('ebook','gyms')->firstOrFail();
    ...
    public function workoutEbookDownload(Request $request)
    {
        $download = WorkoutEbookDownload::firstOrCreate($request->all());

        $download->ebook->update(['download_total' =>  count($download->ebook->downloads)]);

        $download->notify(new WorkoutEbookDownloadNotification($download->ebook->url));
